...
from ai_engine.route_engine import (
    ...
    calculate_route_risk, classify_patrol_utility
)
from ai_engine.route_analyzer import explain_patrol_utility
from utils.style_loader import load_styles
from components.sidebar import render_sidebar

st.set_page_config(page_title="AI Crime Intelligence | Patrolling Agent", page_icon="🚓", layout="wide")
load_styles()
render_sidebar()
...
<div style="padding: 10px 0 30px 0;">
    <h1 style="font-size:2.4rem; margin-bottom:4px;">🚓 AI Patrolling Agent</h1>
    <p style="color:#7c83a8; font-size:1rem; margin-top:0;">
        Crime-maximizing patrol route recommendation for law enforcement powered by Google Gemini AI
    </p>
</div>
...
with st.form("route_form"):
    ...
    with col3:
        use_ai_explain = st.checkbox("🤖 Generate AI Patrol Utility Explanation", value=True,
...
if submitted:
    ...
    for route in routes:
        ...
        classification = classify_patrol_utility(risk_data["risk_score"])
        scored_routes.append({**route, **risk_data, **classification})

    # Sort: highest utility first (highest risk score, then most high priority, then most crimes, then shortest duration)
    ...
    # ── AI Explanations ──────────────────────────────────────────────────────
    if use_ai_explain:
        with st.spinner("🤖 Gemini AI is analyzing patrol utility..."):
            for r in scored_routes:
                r["ai_explanation"] = explain_patrol_utility(
    ...
    # Draw routes (reverse order so best is on top)
    for route in reversed(scored_routes):
        folium.PolyLine(
            locations=route["coords"],
            color=route["color"],
            weight=5 if route["risk_score"] >= 70 else 4,
            opacity=0.9 if route["risk_score"] >= 70 else 0.6,
            tooltip=f"{route['level']} | Patrol Utility: {route['risk_score']}/100 | {route['distance_km']} km"
        ).add_to(m)
    ...
    st.markdown(f"""
    <div style="background:linear-gradient(135deg, rgba(59,130,246,0.12), rgba(6,182,212,0.08));
                border:1px solid rgba(59,130,246,0.3); border-radius:14px; padding:16px 20px;
                margin-bottom:24px; display:flex; align-items:center; gap:16px;">
        <span style="font-size:2rem;">🚓</span>
        <div>
            <p style="color:#60a5fa; font-family:'Syne',sans-serif; font-weight:800;
                      font-size:1rem; margin:0 0 2px 0;">AI RECOMMENDED PATROL ROUTE</p>
            <p style="color:#f0f0ff; font-size:0.9rem; margin:0;">
                Route #{best['index']+1} — {best['distance_km']} km · {best['duration_min']} min ·
                Patrol Utility: <b style="color:#60a5fa;">{best['risk_score']}/100</b>
            </p>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    for i, route in enumerate(scored_routes):
        ...
        bg_alpha = "0.10" if route["risk_score"] >= 70 else "0.08"
